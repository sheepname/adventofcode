...
int main()
{
    ...
    sort(reseaux.begin(), reseaux.end(), [](const unique_ptr<reseau> &r1, const unique_ptr<reseau> &r2)
         { return r1->junctions.size() > r2->junctions.size(); });

    long long compteur = 1;
    for (int i = 0; i < 3; i++)
    {
        cout << "Reseau " << i << " taille : " << reseaux[i]->junctions.size() << endl;
        compteur *= reseaux[i]->junctions.size();
    }
    cout << "Resultat : " << compteur << endl;
